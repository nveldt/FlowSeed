%% This code will build a graph from an MRI
% Developed by David Gleich with Nate Veldt 2016-01-22
%

function [ei,ej,evi,evd,N,Nmap] = graph_from_mri(X,r)
% X is the MRI image
% n1-by-n2-by-n3
            ...
            for oi=-r:r
                for oj=-r:r
                    for ok=-r:r % even more loops!
                        ...
                        xi = min(sqrt(double(X(i,j,k)))/63,1);
                        xj = min(sqrt(double(X(u,v,w)))/63,1);
...
%fprintf('graph_from_mri: assembling sparse matrix ...\n');

% see note above about ei,ej,ev.
%A = sparse(ei,ej,ev,nnodes,nnodes);
