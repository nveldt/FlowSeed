# This is self-contained Julia code for minimizing the seed-penalized
# conductance score.
# This flow-based method that computes the set S which minimizes a
# localized variant of the conductance objective.
#
# The main subroutine is LPR = Local Push Relabel.
# Unlike previous local flow methods, this repeatedly updates Phase 1 of the
# push-relabel maximum s-t flow algorithm. This phase returns a minimum cut,
...
# Given a graph adjacency matrix A, and a set of nodes R, find the neighborhood
# of radius R around the seed set and output it.
# This is a very inefficient way to do this, and should eventually be replaced.
function neighborhood(A::SparseMatrixCSC,R::Array{Int64},radius::Int64)

    N = R

    for i = 1:radius
        N = unique([findnz(A[N,:])[2]; N])
...
function set_stats(A::SparseMatrixCSC{Float64,Int64},S::Vector{Int64},volA::Float64)
...
# Compute the s-t cut score corresponding to a set S.
function cutval(A::SparseMatrixCSC{Float64,Int64},S::Vector{Int64},R::Vector{Int64},d::Array{Float64,2},alpha::Float64,epsilon::Float64,volA::Float64,pR::Array{Float64},RinS::Array{Float64})
...
#
#   deg = unweighted degree vector. List of number of neighbors of each node
#
# volA, volR = volumes of the entire graph and seed set respectively
function FlowSeed(A::SparseMatrixCSC{Float64,Int64},R::Vector{Int64},Rn::Vector{Int64},Rc::Vector{Int64},epsilon::Float64,pR::Array{Float64},RinS::Array{Float64},
    d::Array{Float64},localFlag::Bool=true)

    println("FlowSeed: Flow-Based Method for Finding Low-conductance Output, with penalties on excluding seed nodes")

    # As long as you are careful when with your input, there is no need
    # to have this be part of the runtime every time this method is called.
    if ~issymmetric(A)
        println("This code only works on undirected networks.")
        SL = 0
        cond = 0
        return SL, cond
    end
    ...
    # Put the graph in adjacency list form. This is expensive for very large
    # graphs, but a one-time comptutation, and will save time later in the
    # push relabel function.
    #
    # deg is the unweigted degree vector; it counts number of neighbors, not
    # weight of edges (this matters if A is a weighted graph)
    if localFlag
        if volA*epsilon/volR < 10
            println("Note that vol(R)/epsilon = O(vol(G)). For these parameters \nit may be faster to run the algorithm without the locality setting.")
        end
    end

    # Call nodes that must be S the "strong seed nodes"
    localStrong = find(RinS)
    ...
    while alphaCurrent < alph0
        ...
        if length(S) > 0 && length(S) < n
            ...
            # You could just measure conductance
            # Stats = set_stats(A,S,volA)
            # alphaS = Stats[4]

            # But we're trying to optimize a localized conductance score
...
#
# deg = the unweighted degree vector for nodes. Counts number of neighbors.
#
# R = a list of nodes that share an edge with the source node
#
# Rn = a list of nodes not in R that neighbor a node in R
function LocalPushRelabel(A::SparseMatrixCSC{Float64,Int64},R::Vector{Int64},sWeights::Array{Float64},tWeights::Array{Float64},Rn::Array{Int64})
    ...
    # deg is the number of neighbors of each node. This is only the same
    # as vector d when A is unweighted
    ...
    # or sink-side edge.
    tToL = reshape(tWeights[Local2Global],Lsize,1)
    sToL = reshape(sWeights[Local2Global],Lsize,1)
...
function NonLocalPushRelabel(A::SparseMatrixCSC{Float64,Int64},R::Vector{Int64},sWeights::Array{Float64},tWeights::Array{Float64})
...
function Main_Push_Relabel(C::SparseMatrixCSC{Float64,Int64},F::SparseMatrixCSC{Float64,Int64},ExcessNodes::Array{Int64},excess::Array{Float64})
...
function discharge!(C::SparseMatrixCSC{Float64,Int64},F::SparseMatrixCSC{Float64,Int64},Queue::Vector{Int64},u::Int64,uNeighbs::Array{Int64},height::Array{Int64},excess::Array{Float64},n::Int64,du::Int64,inQ::Array{Bool})
...
# height[u] = height[v] + 1
function relabel!(C::SparseMatrixCSC{Float64,Int64},F::SparseMatrixCSC{Float64,Int64},Queue::Vector{Int64},u::Int64,uNeighbs::Array{Int64},height::Array{Int64},du::Int64,n::Int64)
...
function pushflow!(C::SparseMatrixCSC{Float64,Int64},F::SparseMatrixCSC{Float64,Int64},Queue::Vector{Int},u::Int64,v::Int64,excess::Array{Float64},height::Array{Int64},inQ::Array{Bool},n::Int64)
