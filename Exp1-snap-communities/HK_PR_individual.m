...
for graph = datasets
    ...
    outputfile = strcat('Output',num2str(percentofset),'/',char(graph),'_hkpr_individual.txt');
    outputmatrix = strcat('Output',num2str(percentofset),'/',char(graph),'_hkpr_individual.mat');
    ...
    for commID = 1:numcom
        ...
        % Use each known target node as an individual seed, and take the 
        % lowest conductance output
        PR = [];
        condPR = 1;
        tic
        for rr = 1:numel(R)
         for tol = tols
            ...
            [aPR,acondPR,acutPR,avolPR,y] = pprpush_weighted_mex(A, d, R(rr), tol, alpha,-1-1.e-4);
            if acondPR < condPR
                condPR = acondPR;
                PR = aPR;
            end
        ...
        tic
        for rr = 1:numel(R)
            [HKtemp, condtemp, cutHK, volumeHK] = hkgrow(A,R(rr));
            [ptemp,rtemp,ftemp] = AdjustedPRF(Target,HKtemp,[]);
            if condtemp < condHK
                 fHK = ftemp;
                 rHK = rtemp;
                 pHK = ptemp;
